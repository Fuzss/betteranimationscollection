...
/**
 * Modified copy of ModelSlime as model parts are inaccessible in the vanilla class
 */
...
public class ModelJigglySlime extends ModelBase
{
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        if (this.slimeRightEye != null && entityIn instanceof EntitySlime) {
            ...
            float progress = limbSwing + ageInTicks * (float) FeatureSlime.squishiness / 15.0F;
            float slimy = ((EntitySlime) entity).squishAmount;

            float magnitude = (float) FeatureSlime.squishiness / 20.0F;
            if (slimy < 0.0F) {
