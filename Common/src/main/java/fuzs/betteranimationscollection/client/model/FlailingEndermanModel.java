...
public class FlailingEndermanModel<T extends LivingEntity> extends EndermanModel<T> {
    ...
    public static LayerDefinition createAnimatedBodyLayer() {
        ...
        PartDefinition partDefinition1 = partDefinition.addOrReplaceChild("right_arm", CubeListBuilder.create().texOffs(56, 0).addBox(-1.0F, -1.0F, -1.0F, 2.0F, 2.0F, 2.0F), PartPose.offset(-3.0F, -13.0F, 0.0F));
        for (int i = 0; i < ENDERMAN_ARM_LENGTH; i++) {
            partDefinition1 = partDefinition1.addOrReplaceChild("right_arm_part" + i, CubeListBuilder.create().texOffs(56, 2 + i * 2).addBox(-1.0F, 0.0F, -1.0F, 2.0F, 2.0F, 2.0F), PartPose.offset(0.0F, i == 0 ? 1.0F : 2.0F, 0.0F));
        }
        PartDefinition partDefinition2 = partDefinition.addOrReplaceChild("left_arm", CubeListBuilder.create().texOffs(56, 0).mirror().addBox(-1.0F, -1.0F, -1.0F, 2.0F, 2.0F, 2.0F, true), PartPose.offset(5.0F, -13.0F, 0.0F));
        for (int i = 0; i < ENDERMAN_ARM_LENGTH; i++) {
            partDefinition2 = partDefinition2.addOrReplaceChild("left_arm_part" + i, CubeListBuilder.create().texOffs(56, 2 + i * 2).addBox(-1.0F, 0.0F, -1.0F, 2.0F, 2.0F, 2.0F, true), PartPose.offset(0.0F, i == 0 ? 1.0F : 2.0F, 0.0F));
        }
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (this.creepy && (!this.carrying || FlailingEndermanElement.whileCarrying)) {
            ...
            if (!this.carrying) {
                ...
                for (int i = 0; i < ENDERMAN_ARM_LENGTH; i++) {
                    float armPartZRot = Mth.sin(ageInTicks * animationSpeed * 7 + (float) i * 0.45F) * ((float) (i + 8) / 8.0F) * animationSpeed;
